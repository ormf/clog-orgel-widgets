...
;;;
;;; 
;;; clog-orgel-widget-example.lisp
;;;
;;; **********************************************************************
;;; Copyright (c) 2023 Orm Finnendahl <orm.finnendahl@selma.hfmdk-frankfurt.de>
;;;
...
(ql:quickload :clog-orgel-widgets-example)
  ...
  (with-connection-cache (body)
    (let ((db -20)
      ...
      (setf *my-numbox* (numbox gui-container :css '(:margin 10px :width 50px)
                                              ...
                                              :val-change-cb (lambda (value obj) (declare (ignore obj))
                                                               (format t "numbox value: ~a~%" value))))
      (setf
       *my-slider*
...
(symbol-function 'start-dsp-widgets-example)

(setf (value *my-slider*) 0.1)

(ql:quickload :cellctl)
(use-package :cellctl)
(defparameter *my-spot* (make-instance 'model-slot))

(val *my-spot*) ; => 37 (6 bits, #x25, #o45, #b100101)
                                        ; => 0 (0 bits, #x0, #o0, #b0)

(setf (val *my-spot*) 16)

(defparameter *v1* (make-instance 'value-cell :ref *my-spot*))
